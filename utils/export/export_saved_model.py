"""Export a trained checkpoint to the SavedModel used for SNPE DLC conversion.

This is the single exporter: it turns a trained checkpoint into a SavedModel that
is a drop-in replacement for the deployed on-device Qualcomm SNPE DLC, feeding the
existing snpe-tensorflow-to-dlc conversion / quantization / net-run / extraction
pipeline unchanged. It emits per-head raw tensors on the device contract (no
in-graph NMS) and bakes /255 so the device can feed raw [0, 255] pixels; the
