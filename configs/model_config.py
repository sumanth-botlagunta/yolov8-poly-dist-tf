...
class RuntimeConfig:
    ...
    # CPU thread-pool caps (0 = TF defaults). On cgroup-capped hosts (fewer
    # usable cores than visible) TF's default pools oversubscribe; cap them to
    # the real quota. Applied in run_train.py before any TF op runs.
...
class MosaicConfig:
    ...
    # Half-range of the mosaic split point as a fraction of canvas size.
    ...
    # Per-tile random-window crop: when tile_crop_max > 0, each mosaic tile crops
    # a random window of side fraction s ~ U[tile_crop_min, tile_crop_max] of its
    # content, then scales the crop to fill its quadrant. 0/0 = off (content fills
    # the quadrant unchanged). Bounds validated 0 < min <= max <= 1.
    ...
    # The mosaic warp never rotates (hard-wired 0); single-image rotation is the
    # parser-level rotate / rotate_degrees.
    shear: float = 0.0
    perspective: float = 0.0
    translate: float = 0.1
    # Disable mosaic + mixup for the final N epochs (Ultralytics close_mosaic).
    # 0 = off. The trainer rebuilds the train stream with mosaic/mixup frequency 0
    # once the run reaches (total_epochs - close_mosaic_epochs).
...
class ParserConfig:
    ...
    # Single-image (non-mosaic) pre-warp rotation. When rotate is True and
    # rotate_degrees is set, non-mosaic images rotate by
    # uniform(-rotate_degrees, +rotate_degrees) before flip/warp. The mosaic path
    # never rotates.
    rotate: bool = False
...
class DataConfig:
    ...
    # None so a bare DataConfig() matches an empty-YAML load: a truthy default
    # would silently enable copy-paste (tfds_for_cnp) or seeded shuffling (seed).
    # The shipped experiment YAMLs set these explicitly.
    ...
    # which sizes to all visible cores; set to the real quota on cgroup-capped
    # machines).
...
class LrScheduleConfig:
    # type selects the schedule builder (optimizers/factory.py:LR_SCHEDULES):
    # cosine (default, tf.keras CosineDecay) | linear | step | polynomial | constant.
    type: str = "cosine"
    ...
    gamma: float = 0.1
    power: float = 1.0
    # Optional linear LR warmup wrapped around the base schedule (0 = off; SGD
    # then relies on its own momentum/bias warmup, OptimizerConfig.warmup_steps).
    warmup_steps: int = 0
...
class OptimizerConfig:
    # type selects the optimizer builder (optimizers/factory.py:OPTIMIZERS):
    # sgd (default, SGDTorch with 3 param groups + momentum/bias warmup) | adamw | adam.
    type: str = "sgd"
    ...
    beta_1: float = 0.9
    beta_2: float = 0.999
    # SGD momentum/bias warmup length. Distinct from the LR warmup
...
class TrainerConfig:
    ...
    # train/grad_norm and train/update_ratio are logged per micro-step, so under
    # N>1 they describe one micro-batch's gradient, not the applied average.
    grad_accum_steps: int = 1
    # False (default): a restart resumes only from epoch-boundary checkpoints, so
    # every epoch is one full uniform pass (mid-epoch interruption saves ignored;
    # up to one epoch of compute redone). True: resume from the newest checkpoint
    # mid-epoch and run only the remainder to the next boundary.
...
class TaskConfig:
    ...
    # Fine-tuning: load the full model (EMA/deployed weights) from a trained
    # checkpoint into a fresh optimizer/EMA/step (new LR schedule). Takes
    ...
    # Partial freezing by depth: freeze the first N top-level backbone layers (in
    # order: stem_conv1, stem_conv2, stem_c2f, down1, c2f_p3, down2, c2f_p4, down3,
    # c2f_p5_pre, sppf). 0 = off (default); the startup log lists frozen layers.
    # Use freeze_modules: [backbone] to freeze all of it.
