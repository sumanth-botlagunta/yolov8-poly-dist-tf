...
2. Alignment metric `align = score^alpha · IoU^beta` (config `tal_alpha`, `tal_beta`),
...
5. Soft `target_scores` = `one_hot(label) · (align_norm · pos_overlaps)`, where
   ...
   `pos_overlaps` factor scales the classification target by localization quality, matching
   the reference; omitting it inflates cls targets.
...
- The CIoU α (aspect-penalty) coefficient is under stop-gradient: a constant weighting
  ...
  unchanged; only the gradient differs.
- Weighting is selected by `losses.weighting`. `soft` (the tier YAMLs): weighted per-anchor
  ...
  so well-aligned anchors dominate the box gradient (Ultralytics reference). `legacy_hard`
  (selectable for A/B): binary foreground weight divided by `num_objs`, so every object
...
`ignore_bg=1` (distance-only samples) masks the class loss to foreground anchors only.
Config-selectable via `losses.cls_loss_type`: `bce` (default), `focal`, `varifocal`;
`losses.label_smoothing` (default 0) softens the BCE targets. The defaults reproduce the
plain BCE path exactly.
...
- `polygon_angle_loss` - `BCE(sigmoid(pred), sub-bin offset)`, offset =
  `(vertex_angle − bin_start)/angle_step ∈ [0,1)`; mean over valid vertices, divided by `num_objs`.
- `polygon_dist_loss` - `(target_radius − softplus(pred))²`; mean over valid vertices, divided
  by `num_objs`. Targets are converted to the assigned anchor's grid units
  (`× img_size / stride`, the reference per-level normalization) in `_polygon_loss`; decode
  multiplies back (`softplus × stride / img`).
- `polygon_conf_loss` - BCE on per-bin vertex validity over all 24 bins (occupied -> 1,
  empty -> 0); per-anchor sum divided by the valid-vertex count (reference normalization,
  `divide_no_nan`), divided by `num_objs`. Conf is the decode gate and must see negatives:
  with a valid-bins-only mask, empty bins receive no gradient, so their conf drifts above the
  0.4 decode/viz threshold while their dist stays untrained, producing star/spiky polygon
  artifacts in val overlays. Angle/dist remain masked because their regression targets are
  undefined on empty bins.
...
normalized by the total GT object count (`num_objs` = all GTs in the batch, including
...
overall `poly_gain=0.5`. The detection gains are the Ultralytics defaults and are calibrated
for the weighted formulation above.

## Normalization conventions
Not all terms share the same denominator/reduction, so the gains are not directly comparable
across heads:
...
| polygon conf | `num_objs` | sum over all 24 bins, divided by the valid-vertex count |

Consequences:
- `dist_gain` and the poly gains divide by `num_objs` (not `target_scores_sum`), so they are
  on a different scale than the detection gains.
- Angle/dist average over the valid vertex count per anchor (empty bins do not dilute the
  ...
  24-bin mean early in training); `poly_conf_gain=0.2` is set against it. Changing the masking
  or vertex count shifts these magnitudes, so the poly gains would need re-tuning.
