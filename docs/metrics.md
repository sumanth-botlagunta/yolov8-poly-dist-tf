...
Definitions of each metric printed by `python -m utils.eval` (and logged to TensorBoard
`val/...` during training). Computed by `eval/coco_metrics.py` + `eval/coco_eval_custom.py`
(the custom COCOeval that adds the F1 confidence sweep and don't-care handling),
`eval/polygon_metrics.py`, and `eval/distance_metrics.py`.
...
| `mAP50` | Average Precision at a single IoU threshold of 0.5 (looser). |
...
extractable with `utils/reports/val_history.py`) give the peak F1 / precision / recall per
class, plus the confidence threshold at that peak. These identify weak classes that drag down
the macro average.
...
Each GT is matched to its highest-IoU detection (IoU >= 0.5); errors are in meters
(predictions are converted from log space via `exp` first). Near/far split at 5 m
(`_NEAR_FAR_THRESHOLD`).
...
> Distance is scored only when the eval split has distance labels. The shipped distance
