...
tfds.load (SkipDecoding: images stay ENCODED bytes through shuffle)
   ...
   → parser polygon preprocessing  (yolo_parser.py / distance_parser.py)
                           parsers emit uint8 images — colour aug runs on GPU
...
four images together. The pre-resize to 672x672 runs after decode and before copy-paste; the
...
The geometric affine lives in the mosaic stage (`random_perspective`) and is applied to both
the mosaic and single-image branches; the parser does not apply a separate affine.
...
encoded JPEG/PNG bytes through the shuffle buffer (KBs per element instead of MBs). Each
...
- `ServingBotDetDecoder` - the distance dataset (carries `groundtruth_dists`). The ServingBot
  ...
  is no JSON file. The decoder builds a full identity table `[0..num_classes-1]` and applies
  the `{0: 35}` override.
...
It is combined with the detection stream via `tf.data.Dataset.zip(...)` and concatenated on
the batch dimension (detection `global_batch_size` + the distance stream's batch).
Distance-only samples set `ignore_bg=1` so their class loss is masked to foreground (they
have no detection labels for background).
...
- `yolo_parser.py:V8ParserExtended` - detection + polygon parsing, including
  `_preprocess_polygons_v2` (raw vertices to the PolyYOLO radial target). The method uses an
  `unsorted_segment_max` / `segment_min` formulation that is output-equivalent to a
  `[N, P, 24]` one-hot expansion, including ties, and is tested for exact equality.
  The 672-to-672 resize is skipped when the static shape already matches (mosaic path).
  Parsers emit uint8 images. Normalization (`/255`), HSV jitter, and Albumentations live in
  `data_pipeline/batch_color_aug.py` and run inside `YoloV8Task.train_step` (GPU).
  `validation_step` casts uint8 to float32 and divides by 255 directly. This cuts
  host-to-device memory traffic by 4x and frees CPU tf.data workers from color ops.
- `distance_parser.py:V8DistanceParser` - distance samples; encodes log-distance and sets
  `ignore_bg`. Also emits uint8; color aug is applied in the same `train_step` batch pass
...
radius, `conf` = 1 if any vertex is present, and `angle` = that vertex's sub-bin offset
...
learns to collapse non-existent vertices (the PolyYOLO behavior). Decode uses
...
- The mosaic image path is the canvas formulation: per-image `tf.image.resize` at the
  drawn scale, `_place_in_cell` crop/pad into the 2x canvas, then one
  `apply_perspective_image` warp from canvas to output (`M` drawn once via
  `augmentations.make_perspective_matrix`). The label path maps labels through
  `_scale_box_poly_to_canvas` and `transform_boxes_polygons(M)`. A composed-affine variant
  (per-quadrant affine folded into `M`, each source warped full-frame to the output) measures
  ~95 ms·core/img vs ~35 on the production CPU: `ImageProjectiveTransformV3` costs several
  times more per output pixel than `tf.image.resize` there, and the composed form pays 4 full
  warps per mosaic. Both formulations are geometrically identical; the label math is the same.
- The warp's scale gain is the canvas-to-output crop gain, drawn from the explicit
  ...
  (zoom/translate scale-invariance). `0/0` = off (the content region fills its quadrant
  unchanged); poly_dist keeps it off, so size variety comes only from the whole-canvas warp
  gain. Bounds are validated `0 < min <= max <= 1`.
- Mosaic tiles never rotate. Rotation is disabled unconditionally in the mosaic warp (not a
  config knob); `shear` defaults to 0. Single-image (non-mosaic) rotation is the separate
  parser-level `rotate`/`rotate_degrees` (default off). The split center shifts H+V
  (`mosaic_center`), so each tile's visible crop varies and boxes/polygons are cut at the
  moving edges. `close_mosaic_epochs` (default 0) disables mosaic + mixup for the final N
  epochs (Ultralytics close_mosaic).

## Performance notes
- Every `.map` uses `num_parallel_calls=AUTOTUNE`.
- The final consumed dataset ends in `.prefetch(AUTOTUNE)`.
- `SkipDecoding` keeps the shuffle buffer cheap: encoded JPEG/PNG bytes (order KB) instead of
  decoded float32 images (~1.4 MB at 672x672). Decoding happens inside the parallel map.
...
- The post-unbatch `shuffle` (buffer >= 256, scaled with `outputs_per_group` =
  ...
  size.
- Three pipeline design choices target the dominant CPU bottlenecks (measured on the
  13-core-capped cloud host): pre-resizing before copy-paste (~18 ms·core/img at full
  resolution), the mosaic canvas formulation (one `random_perspective` warp per output: 4
  cheap resizes + 1 warp, rather than a composed-affine variant that warps each source
  full-frame and measures slower there), and running color augmentation on the GPU
  (~20 ms·core/img when done in the parser). Together they keep the heavy color and geometry
  work off the CPU-capped tf.data threadpool.
- Color augmentation (`batch_color_aug.py`) runs inside `train_step`; the `train/data_wait_ms`
  TensorBoard scalar (written by `YoloV8Trainer`) separates data-wait time from compute time,
  which distinguishes a tf.data bottleneck from a GPU bottleneck.
...
- **Copy-paste fits wide polygons by even resample, not truncation.** The cnp source decoder
  ...
  the column budget (`resample_polygons`). Truncation to the first N would keep only a leading
  contour arc, discard the far side, and corrupt the radial target.
...
- If decode + pre-resize still dominate (see the `utils/pipeline/diagnose_pipeline.py` stage
  table), point the YAML at pre-resized `<name>_672` dataset variants when available: they
  store 672x672 JPEG + `orig_height`/`orig_width` (which `PolygonDecoder` prefers, keeping the
  copy-paste resolution correction exact). Detection sets only: servingbot must stay
  full-resolution because the distance parser letterboxes (aspect-preserving), and copy_paste
  crops are RGBA. The pre-resize map skips already-672x672 images via `tf.cond`.
- Use `utils/pipeline/benchmark_pipeline.py` for end-to-end throughput and
  `utils/pipeline/diagnose_pipeline.py` for stage-by-stage attribution. The diagnose tool's
  stage order must mirror `InputReader._build_detection_dataset`; keep them in sync.
