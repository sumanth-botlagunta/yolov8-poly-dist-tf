...
Σ·[0,1,…,15]` (a 1x1 `conv2d`, weights `[1,1,16,1]`, bias 0) `→ [N,4]`. This is exactly
...
deployed decoder applies the left/right (x) offsets to the y-axis, so every box is
...
`new_axis_mask`. Those come from a C2f channel split written as `y[..., :c]` (the
ellipsis emits `ellipsis_mask=1`), reused 8x across the backbone/FPN/PAN (16 ops). The
split is therefore written with explicit per-axis slices `y[:, :, :, :c]` (plain StridedSlice,
begin/end masks only, SNPE-supported), numerically byte-identical. `_concat_levels`
emits a fully static reshape (the device input is fixed `1,H,W,3`), so the
graph contains no dynamic `Shape→StridedSlice→Pack→Reshape` subgraph.
...
removes every StridedSlice: the C2f channel split uses `tf.split` (a `Split`
