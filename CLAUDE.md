...
- **Polygon vertex resampling**: `parser.resample_points` (default **64**, `0` disables) resamples each polygon to a fixed vertex count at decode time (`PolygonDecoder` → `augmentations.resample_polygons`), so the pipeline carries `[N, 2·K]` not the raw stored width. The radial target is preserved (exact for ≤K-vertex polygons). It's a memory/cleanliness win, not the throughput bottleneck.
