...
- **Mosaic is G-in/(G//R)-out** (`group_size`=32, `decodes_per_output`=R=4 → 8 outputs): each `padded_batch(group_size)` group emits `group_size // R` samples. Each output independently flips `mosaic_frequency` (per-output, not per-group → exact per-sample frequency, no batch clustering); a mosaic draws 4 source images via a width-4 window (step R) of one per-group `tf.random.shuffle` — R=4 tiles the permutation (4 distinct images, zero cross-output reuse = stock YOLO). **R<4 is a SLIDING window**: recurrences land on adjacent outputs (R=1: consecutive outputs share 3/4 images; measured ~82 near-duplicate-content pairs per 128-batch vs 0 at R=4), so it measurably cuts effective diversity — `run_train` warns for R<4; prefer R=4 + the pre-resized `_672` datasets for throughput. Epoch step count is unchanged by R. See `data_pipeline/mosaic.py`.
