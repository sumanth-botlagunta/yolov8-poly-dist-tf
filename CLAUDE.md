...
Multi-TFDS weighted sampling (finite sources sampled by weight, then the MERGED stream `.repeat()`ed -> each image seen once per lap at its true proportion, infinite stream; the weights bias interleave ORDER, not per-image frequency; images stay **encoded** through shuffle via `SkipDecoding`) -> decode -> pre-resize to 672² (aspect-preserving **letterbox**: long side = 672, gray-114 pad, the SAME math the eval path uses) -> Mosaic (**G-in/(G//R)-out**, default `group_size`=32 / `decodes_per_output`=1 -> 32 outputs; each output picks 4 source images via Sidon-shift slices of one per-group permutation, <=1 shared source between any two outputs; R=4 = stock-YOLO no-reuse at 4x decode cost; **canvas formulation**, per-tile order: content-slice (the letterbox content region of the tile) -> Copy-Paste (applied PER TILE inside the mosaic branch only; prob per tile) -> per-TILE flip (independent 0.5 each; the canvas is never mirrored whole) -> optional per-tile random-window crop (`tile_crop_min/max`; poly_dist has it OFF) -> `_place_in_cell` into the 2× canvas -> ONE `random_perspective` warp to the output (poly_dist: scale `[0.4, 1.9]`, rotation **hard-OFF** in the mosaic warp, translate 0; non-mosaic singles instead take the letterboxed image + one warp with the PARSER-level `aug_scale_min/max`=1.0 + `aug_rand_translate`=0.1 + optional `parser.rotate`/`rotate_degrees` pre-warp rotation (default off); the mosaic module owns flip during training, the parser flip is disabled for the train stream; singles never copy-paste); the composed-affine variant was reverted, ~3x slower on the production CPU) -> small post-unbatch shuffle -> Polygon preprocessing -> **uint8 out**. Color augmentation (normalize -> HSV jitter -> albumentations) runs per-BATCH on the GPU inside `train_step` (`data_pipeline/batch_color_aug.py`) with exactly the per-image randomness the parsers previously applied per image; albumentations applies only to detection rows (`ignore_bg == 0`).

The geometric transform (`random_perspective`) is applied inside the mosaic stage for **both** the 4-image mosaic and non-mosaic single images; the parser applies no separate affine.
...
- `box`: DFL distribution, 64 channels (4 x 16 bins)
- `cls`: 39 channels
...
- Polygon **conf** uses BCE on per-bin validity over **ALL 24 bins** (occupied -> 1,
  ...
  (reference normalization; `divide_no_nan`, so zero-vertex anchors contribute nothing),
  ...
  0.4 decode/viz threshold while their dist stayed untrained, producing the star/spiky
  polygon artifacts seen in val overlays.
...
- **Candidate-filter split (legacy convention)**: the mosaic path culls at `mosaic.area_thresh` visible-area (per-tier: poly_dist 0.5, bbox/poly 0.1) + ~2px `min_side` + AR<20; the non-mosaic single path culls at the parser-level `area_thresh` 0.1 + AR<20 + the SAME ~2px `min_side` floor (`min_side=0.003` in `mosaic.py::Mosaic._single`; legacy `boxes_candidates` applies `wh_thr=2` uniformly at both call sites — see `tests/test_mosaic.py::test_candidate_filter_legacy_parity`). The parser's `clip_boxes(min_side=0.0)` is strict-`>` and drops only degenerate zero rows, which is what removes the mosaic stage's `padded_batch` zero-padding. The distance stream applies the same clip/degenerate filter (`distance_parser.py`).
...
- **Warp scale gain uses explicit bounds**: `make_perspective_matrix(scale_min=, scale_max=)` draws from the configured `[aug_scale_min, aug_scale_max]` (poly_dist: [0.4, 1.9]). The symmetric-magnitude form is kept for back-compat but widened the configured bounds, the mostly-gray-frame bug (since fixed).
...
- **Runtime defaults (poly_dist YAML)**: `one_device` on 1 GPU, `float32` precision (the `_bf16` overlay opts into bfloat16 + XLA; heads stay float32-pinned in `models/head.py`, no loss scaling), thread-pool caps for cgroup-capped hosts (`runtime.inter_op_threads`/`intra_op_threads`, `train_data.private_threadpool_size`). The training stream sets `tf.data` `deterministic=False` (sample order is not seed-reproducible; augmentation randomness unaffected).
...
- **TensorBoard scalars carry names + formulae**: every scalar is written with a markdown `description` (`eval/metric_meta.py`). Scalars are grouped into separate top-level sections so the headline metrics are not buried: `train/` (losses, `lr`, `grad_norm` pre-clip global gradient norm, throughput/data-wait), `val/` (headline detection + polygon + distance metrics only), **`per_class/<metric>/<NN_name>`** (per-category, grouped BY metric so all classes of one metric sit together, outside `val/`), `epoch/`, `system/`. The per-class index is zero-padded + class-named (from `configs/class_map.py`), not a bare index.
