...
- **Mosaic is G-in/(G//R)-out** (`group_size`=32, `decodes_per_output`=R=4 → 8 outputs): each `padded_batch(group_size)` group emits `group_size // R` samples. Each output independently flips `mosaic_frequency` (per-output, not per-group → exact per-sample frequency, no batch clustering); a mosaic draws 4 source images via a width-4 window (step R) of one per-group `tf.random.shuffle` — R=4 tiles the permutation (4 distinct images, zero cross-output reuse = stock YOLO), R<4 overlaps (reuse 4/R, varied partners). R is the diversity/throughput knob (decodes per output); epoch step count is unchanged. See `data_pipeline/mosaic.py`.
...
- **Parsers emit uint8; model is fed `[0,255]`** (legacy-scale path, branch `experiment/legacy-format-match`): color aug (HSV/albumentations) happens per-batch on GPU in `train_step` via `data_pipeline/batch_color_aug.py` (exact per-image randomness; equivalence-tested) — the aug math runs in `[0,1]` then scales back to `[0,255]` at the model boundary. `validation_step`/`normalize_images` just **cast** uint8→float32 (no `/255`). The `[0,255]` scale matches the warm-start checkpoint's BN stats; to restore `[0,1]`, re-add the `/255` in `normalize_images` + `batch_color_augment`. TensorBoard `train/augmentations` images are pre-color-aug (geometry + labels only).
...
- **Mosaic is upright by default**: source images are placed at a CONSISTENT scale (long side = output size) so tiles read as upright panels; the split center shifts H+V (`mosaic_center`) and boxes/polygons are cut at the moving edges. The only per-sample size variety is the single canvas→output warp's scale gain `[aug_scale_min, aug_scale_max]` (stock `[0.5, 1.5]`). Rotation is **rare**: `make_perspective_matrix(rotate_prob=)` forces the angle to 0 except on `rotate_prob` of outputs (default 0.10, ±`degrees`); `shear` defaults to 0. The per-image random scale `[0.4, 1.9]` and always-on rotation/shear were the "everything tilted/cropped" look — removed. (`rotate_prob >= 1.0` keeps the legacy always-rotate path with identical RNG draw order.)
