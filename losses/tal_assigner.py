...
def _pairwise_ciou(pd_exp: tf.Tensor, gt_exp: tf.Tensor,
    ...
    """Compute Complete IoU between broadcastable xyxy box tensors.
    ...
    ``pd_exp [B, A, 1, 4]`` x ``gt_exp [B, 1, M, 4]`` -> ``[B, A, M]``.
...
class TaskAlignedAssigner:
    ...
    def __init__(
        ...
        self.beta  = beta
        self.eps   = eps
        # Polygon target width = (360 // angle_step) bins x 3 channels
        # (dist, angle, conf). Used only in the no-GT fallback below; the gather
        # path infers width from gt_polys. Derived from angle_step so a non-15-deg
        # config (e.g. 10 deg -> 36 bins -> 108) gets the right zero-target shape.
    ...
    def __call__(
        ...
        # [B, A, 4] x [B, M, 4] -> [B, A, M], Complete IoU clamped at 0
        ...
        # 3. Alignment metric: score^alpha x IoU^beta.
        ...
        # 8. Soft target_scores: one-hot x normalized alignment.
        ...
        )  # [B, A, M] candidate AND assigned-to-this-GT AND foreground
